Feature: Exports, reports and sitemap generation
  ...
  Scenario: Exports are discoverable
    When I run "bluesnake export --list"
  ...
  Scenario: Issue-filtered export
    When I run "bluesnake crawl <serverurl>/ --store-dir <storedir> --quiet"
    And I run "bluesnake export <crawlid> response_codes --store-dir <storedir> --filter internal_client_error"
    Then the exit code is 0
  ...
  Scenario: Issues export as JSONL
    When I run "bluesnake crawl <serverurl>/ --store-dir <storedir> --quiet"
    And I run "bluesnake export <crawlid> issues --store-dir <storedir> --format jsonl"
    Then the exit code is 0
  ...
  Scenario: Crawl overview report
    When I run "bluesnake crawl <serverurl>/ --store-dir <storedir> --quiet"
    And I run "bluesnake report <crawlid> crawl_overview --store-dir <storedir>"
    Then the exit code is 0
  ...
  Scenario: Sitemap generation
    When I run "bluesnake crawl <serverurl>/ --store-dir <storedir> --quiet"
    And I run "bluesnake sitemap <crawlid> --store-dir <storedir> -o <storedir>/maps"
    Then the exit code is 0
