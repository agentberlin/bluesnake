...
func (q *Queue) AddRequest(r *bluesnake.Request) error {
	q.mut.Lock()
	waken := q.wake != nil
	q.mut.Unlock()
	if !waken {
...
func (q *Queue) storeRequest(r *bluesnake.Request) error {
	d, err := r.Marshal()
	if err != nil {
		return err
	}
...
func (q *Queue) Run(c *bluesnake.Collector) error {
	q.mut.Lock()
	...
	requestc := make(chan *bluesnake.Request)
...
func (q *Queue) loop(c *bluesnake.Collector, requestc chan<- *bluesnake.Request, complete <-chan struct{}, errc chan<- error) {
	var active int
	for {
...
func (q *Queue) loadRequest(c *bluesnake.Collector) (*bluesnake.Request, error) {
