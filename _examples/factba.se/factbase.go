...
func main() {
	c := bluesnake.NewCollector(
		bluesnake.AllowedDomains("factba.se"),
	)

	d := c.Clone()

	d.OnHTML("body", func(e *bluesnake.HTMLElement) {
		t := make([]transcript, 0)
		e.ForEach(".topic-media-row", func(_ int, el *bluesnake.HTMLElement) {
		...
		os.WriteFile(bluesnake.SanitizeFileName(e.Request.Ctx.Get("date")+"_"+e.Request.Ctx.Get("slug"))+".json", jsonData, 0644)
	})
