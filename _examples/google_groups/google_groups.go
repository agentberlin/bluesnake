...
func main() {
	...
	threadCollector := bluesnake.NewCollector()
	mailCollector := bluesnake.NewCollector()

	// Collect threads
	threadCollector.OnHTML("tr", func(e *bluesnake.HTMLElement) {
		ch := e.DOM.Children()
		author := ch.Eq(1).Text()
		// deleted topic
		if author == "" {
	...
	// Visit next page
	threadCollector.OnHTML("body > a[href]", func(e *bluesnake.HTMLElement) {
		log.Println("Next page link found:", e.Attr("href"))
		e.Request.Visit(e.Attr("href"))
	})

	// Extract mails
	mailCollector.OnHTML("body", func(e *bluesnake.HTMLElement) {
		// Find subject
		...
		// Extract mails
		e.ForEach("table tr", func(_ int, el *bluesnake.HTMLElement) {
			mail := Mail{
