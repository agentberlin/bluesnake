...
func TestLinkScoreAndAggregates(t *testing.T) {
	...
	r := Run(pages, nil, nil, nil, config.Default())
...
func TestSitemapSetOps(t *testing.T) {
	...
	r := Run(toMap(inSitemapLinked, orphan, noindexed, notListed, nestedSitemap), index, nil, nil, config.Default())
