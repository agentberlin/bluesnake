...
// FetchSitemapURLs fetches a specific sitemap URL and returns all discovered URLs.
// It handles both regular sitemaps and sitemap indexes automatically.
// Returns an empty slice if the sitemap cannot be fetched or parsed.
...
// TryDefaultSitemaps tries to fetch sitemaps from common default locations.
...
// This function does not return errors - it returns empty slice if no sitemaps found.
func TryDefaultSitemaps(baseURL string) []string {
	...
	for _, location := range sitemapLocations {
		urls, err := FetchSitemapURLs(location)
