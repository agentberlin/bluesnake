...
- **Old in-RAM finalize twins are kept, test-only (P14, deferred).** `crawler.RecomputeDepths`/`RecomputeInlinks`, `store.SaveDepths`/`SaveInlinks`/`SaveInlinkSources`/`UpdateInlinks`, `c.inlinks`/`Result.Inlinks` are production-unused but back the crawler-package test harness (`capFinalize`). They can't simply be repointed at `finalize.Crawl` — `finalize` imports `crawler`, so a crawler-package test can't import `finalize` (cycle); deleting them is a test-package restructure with zero production behaviour/RAM change, tracked here rather than rushed.
