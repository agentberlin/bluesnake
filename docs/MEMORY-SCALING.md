...
- **`speed.max_concurrent_crawls` parallelises the CLI's `projects crawl-all` only.** MCP and desktop run one crawl at a time by design (their tools/progress UI assume a single current crawl); the knob's copy on those surfaces was relabelled to say so (issue-#73 P6/P7) — it is not silently inert with parallelism-promising copy.
...
| **8** ✅ | DONE (CLI). Executor holds many crawls (`cur map[crawlID]`) with crawl-id `PauseCrawl/StopCrawl` + no-arg fan-out + a shared limiter; Dispatcher runs W drain loops (`queue.WithConcurrency`, atomic `ClaimNext` so no double-claim, wake-the-next, `Shutdown` waits all loops), `Cancel(jobID)`→`StopCrawl` (guarded against the startup-window empty crawlID, issue-#73 P18). Config `speed.max_concurrent_crawls`; `projects crawl-all --parallel N` runs N members at once under one global fetch cap. **MCP/desktop run one crawl at a time by design** (their single-crawl tools/UI), so the knob is CLI-only there and its copy says so (issue-#73 P6/P7); the undriven `SnapshotCrawl`/`CurrentAll` parallel-addressing API was removed (P16). Tests: GL-05/GL-17 (parallel/no-double-claim), GL-13 (shutdown-all), GL-03/GL-04 (crawl-id control). | Med | ✓ |
