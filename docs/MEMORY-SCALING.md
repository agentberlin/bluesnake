...
- **Per-crawl RAM is frontier-LINEAR, not "independent of the frontier."** The dominant terms (parked goroutines, retained records, the in-RAM visited-set) *are* eliminated — the large, real win — but the work queue (`workpool.go`) is an unbounded in-RAM slice, and `c.inlinks` + `Result.Inlinks` are frontier-sized. RAM is frontier-linear with a ~10–15× smaller constant. The §5.2/§5.3 bounded SQLite ready-buffer / feeder (with `claimed`/`seq` frontier columns) was **NOT built** — the shipped Phase 3 is an in-RAM queue. Every §13 row that references `claimed`/`seq` frontier columns describes that **unbuilt** feeder design, not the shipped pool.
...
**Per-crawl RAM bound (PLAN — partially realised):** the §5.2/§5.3 design targets
`O(threads + ready_buffer + bloom + small_counters)`, **independent of frontier and
crawled-page count**. As SHIPPED the ready-buffer/feeder was not built, so the bound
is `O(threads + frontier + bloom)` — frontier-LINEAR with a ~10–15× smaller constant
(the dominant goroutine-stack + retained-record + in-RAM visited-set terms are gone;
the unbounded in-RAM work queue + `c.inlinks` remain frontier-sized). See the
"Honest residuals" note at the top. Measured target was tens of MB (vs ~1.7–1.9 GB
at 4k pages); the realised win is large but not frontier-independent.
