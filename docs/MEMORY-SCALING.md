...
Status: **landed, with the PR-#69 review follow-ups (#70/#71), the issue-#73 remediation, AND the issue-#77 bounded frontier (SQLite feeder + ready-buffer) applied — per-crawl RAM is now flat on the frontier axis as well.** The two biggest memory wins are real and live: the goroutine-per-URL model is gone (bounded worker pool) and page records stream-and-drop instead of being retained. The Phase-2 SQL/CSR finalize body is fully cut over & live: inlinks/discovered_from (gated `edges` table), depth (CSR over `links`), PageRank (CSR via `analyze.WithLinks`), dup-rule (`store.DuplicateIssues`), and the near-dup minhash column (`internal/minhash` leaf pkg + `pages.minhash`, migration v4) — each parity-proven against the in-RAM path and verified by EC24 golden + resume_equivalence + acceptance.

**Review follow-ups now applied (#70/#71):** the store is the frontier dedup authority on *every* surface (runner.sink forwards `frontier.Dedup`), so pause/resume no longer silently loses pages (C1) and the live Discovered counter advances on MCP/desktop (M1); per-bucket admission counters are rehydrated on resume so caps bind across the interrupt boundary (FR-08/M3); the global fetch limiter is derived from `speed.max_global_threads` (H1), the finalize/analysis pass is bounded by the finalize semaphore (H2); the Bloom fast-negative is wired in front of `store.Admit` (H3) and now **sized from `cfg.Limits.MaxURLs`** so it doesn't saturate at multi-million scale (issue-#73 P5); and the `seenContent` short-circuit moved to the on-disk `content_hash` table (M4).

**Issue-#73 remediation applied (correctness + honesty):** finalize seals a crawl `completed` only AFTER depth + analysis are durable, so a crash mid-finalize leaves a resumable crawl, never a `completed` one with stale depths / missing issues (EC-05/P1); `PendingFrontier` excludes already-crawled URLs so a crash-then-resume never re-fetches or double-charges the budget (EC-02/P2); a crawl that predates the gated `edges` table is **refused on resume** with a re-crawl message rather than silently corrupted (P3); `link_score` is now **bit-stable run-to-run** via canonical (sorted) PageRank accumulation (P4); and a real latent bug was fixed — `runner.sink` now forwards the `content_hash` authority (`crawler.ContentSink`), so the M4 bound is actually in effect in production and a resume's canonical owner survives the interrupt (P11).

**Honest residuals (corrected from earlier overstatements — issue-#73 P13/P20):**
- **~~Per-crawl RAM is frontier-LINEAR~~ CLOSED (issue #77).** The §5.2/§5.3 bounded ready-buffer + SQLite feeder is now BUILT: the `frontier` table carries `claimed` + monotonic `seq` (migration v5, index `(claimed, depth, seq)`), rows are **born claimed** inside `Admit` and published claimable only after the cap checks pass (closing the cap-rollback-vs-claim race structurally), and a **single feeder goroutine** — the buffer's only producer — claims `(depth, seq)`-ordered batches into a `clamp(4·threads, 32, 1024)`-item channel that workers drain. Workers never push (WP-03 impossible by construction); only the feeder claims (WP-08 likewise); `Recover` resets orphaned claims at every Run start on every surface (EC-01). The resume loader no longer materialises the `Processed`/`Pending` slices either — the store IS the visited set and the queue — so the frontier-linear term is gone from the resume path too. Measured gate: `TestFrontierRAMSlopeFlat` (test/frontier_feeder_test.go) holds peak-RAM slope ≈ 0 across a 50k-URL frontier delta (−0.2 MB measured) while its detector arm — the same harness forced onto the in-RAM fallback queue — shows the +8.4 MB linear slope the gate forbids. The final frontier-adjacent term — `Resume.Admitted` bucket-counter rehydration, which materialised the admitted set on every per-bucket-capped resume — is now **also CLOSED**: `store.EachAdmitted` STREAMS the admitted union and the loader accumulates only the small `perDepth`/`perSub`/`perPath` maps via `frontier.BucketCounts` (`Resume.Admitted []Item` → `Resume.PerDepth`/`PerSub`/`PerPath`). Gated by `TestResumeBucketCounterRAMFlat` (internal/runner): loadResume retains +0.0 MB across a 50k-row admitted delta, with a detector arm materialising the pre-fix slice at +6.4 MB. Host-key parity of the streamed rehydration is pinned by `TestPerSubRehydration_HostKeyMatchesUrlutilHost` (FR-17).
- **`link_score` determinism comes from sorted accumulation, not a `url→id` int32 CSR.** The deterministic int32-CSR (FIN-CSRID) in §5.4/§7 was not built; instead PageRank accumulates in a canonical sorted node/edge order, which makes the stored value bit-stable (and lets `TestPageRankCSRParity` assert exact equality, no 1e-9 fudge). `discovered_from` first-wins is stable per crawl-DB (seq-MIN, stable across resume) but, like any first-wins under concurrent fetching, is not bit-identical across independent re-crawls at `threads>1`.
- **~~`speed.max_concurrent_crawls` parallelises the CLI's `projects crawl-all` only~~ CLOSED (issue #78).** The knob now has identical semantics on every surface: the CLI resolves config > 1 at command start (the `--parallel` flag was removed 2026-07-05); the desktop app and both MCP servers (standalone + embedded) read it from the default profile at dispatcher construction via `runner.ProcessWiring` and inject ONE shared process-wide limiter (H1/P17 — fetch cap, F=1 finalize, Chrome render pool; since 2026-07-05 the limiter is injected unconditionally and the width is LIVE — `Dispatcher.SetConcurrency`, applied on desktop profile save / re-read at every MCP start, no restart; see DESIGN.md). Per-crawl control is crawl-id-addressed end to end: `Dispatcher.CurrentAll/PauseCrawl/StopCrawl`, `Executor.SnapshotCrawl/Snapshots`, `Observer.OnPage(crawlID, …)`; the MCP tools take an optional `crawl_id` (ambiguous no-id calls error with the live ids), the desktop's events carry the crawl id so parallel progress streams never cross, and an MCP start beyond capacity is rejected naming the running crawls (the blocking per-job start handshake is `Dispatcher.AwaitCrawl`, keyed by job id). The rework also closed a real GL-08 hole it surfaced: the out-of-band crawl-start fetches (`/llms.txt`, `/llms-full.txt`, the sitemap enumeration walk) bypassed the global fetch cap (`crawler.fetchCapped` now takes a slot; robots.txt keeps its documented bypass), and GL-09 for real: the registry needed WAL + `_txlock=immediate` (deferred SELECT→UPDATE upgrades return SQLITE_BUSY *bypassing* busy_timeout) + a serialized fresh-open (the rollback→WAL switch races concurrent first-opens). Default stays 1 — single-crawl behaviour byte-identical.
- **Old in-RAM finalize PATH deleted (P14 done); two parity ORACLES deliberately kept.** The dead parallel finalize *implementation* — `crawler.RecomputeDepths`/`recomputeDepthsOver`/`RecomputeInlinks`/`followsForDepth`, the frontier-sized `c.inlinks`/`noteInlink` chain, `Result.Inlinks`/`InlinkAgg`, and `store.SaveDepths`/`SaveInlinks`/`SaveInlinkSources`/`UpdateInlinks` — is removed. `capFinalize` (the crawler-package test helper) was rewritten to reproduce finalize via the PRODUCTION entry points (`RecomputeDepthsFromLinks` + the gated `edges`), so all ~35 `crawl()`/`runCap()` tests pass unchanged through the shipped path, and per-crawl RAM no longer carries the frontier-sized inlink map. What remains is intentional: `issues.duplicates()` and `analyze.Run`'s in-memory `Facts.Links` mode are NOT dead — they are the independent reference implementations the production SQL/CSR paths are validated against (`TestDupSQLParity`, `TestPageRankCSRParity`). They are also legitimate in-memory library entry points (the analyze unit suite uses `Run(pages,…)` directly). Deleting them would erase the non-circular verification of the shipped paths and reduce library ergonomics for zero production/RAM gain, so they stay.
- **Coverage is Chrome-dependent.** `make cover` now includes the acceptance suite via `-coverpkg` (issue-#73 P19); the absolute number depends on the toolchain (the `render` package needs Chrome — ~88% without it here, ~91%+ on a Chrome-equipped CI), so the earlier "~87.7% / 91.3%" notes are two different toolchains, not a discrepancy.
...
> - **Bounded frontier — resume residual closed (issue #77, 2026-07-03)** — the one frontier-adjacent term #77 shipped with named-but-unfixed: bucket-counter rehydration materialised the whole admitted set (`Resume.Admitted []frontier.Item`) on every per-bucket-capped resume. Now streamed: `store.AdmittedItems` → `store.EachAdmitted(fn)` (a cursor over the same `pages ∪ pending-frontier` union, EC-02 guard intact) feeds `frontier.BucketCounts`, which mirrors `Admit`'s increment block (host via `urlutil.Host`, ByPath first-match) and returns only the small `perDepth`/`perSub`/`perPath` maps; `RehydrateCounters([]Item)` → `SetCounters(maps)`; `Resume.Admitted` → `Resume.PerDepth`/`PerSub`/`PerPath`; `loadResume` computes the maps under `AnyBucketCap`. Gates: `TestResumeBucketCounterRAMFlat` (loadResume retains +0.0 MB across a 50k-row admitted delta; detector arm materialises the pre-fix slice at +6.4 MB), `TestPerSubRehydration_HostKeyMatchesUrlutilHost` (FR-17, previously documented-but-missing). Behavioral equivalence unchanged (`TestResume_NoOverAdmitPerBucket_ThroughRunner`, `TestRehydrateCountersEnforcesCapsOnResume`).
> - **Bounded frontier (issue #77, 2026-07-02)** — the deferred §5.2/§5.3 feeder + ready-buffer, adapted to the shipped pool. `frontier` gains `claimed` + monotonic `seq` (migration v5; index `(claimed, depth, seq)`; seq seeded from `MAX(seq)` at open so a resumed session's admissions sort after the pending tail, EC-07). `store.Admit` writes rows **born claimed=1** — invisible to the feeder — and `frontier.Queue.Enqueue` publishes them claimable only after the cap checks pass, closing the cap-rollback-vs-claim race by construction. The pool (`workpool.go`) became a bounded channel (`clamp(4·threads, 32, 1024)`) whose **single producer** is a feeder goroutine claiming `(depth, seq)`-ordered batches (`UPDATE … WHERE url IN (SELECT … WHERE claimed=0 … ORDER BY depth, seq LIMIT n) RETURNING`, re-sorted in Go — RETURNING order is unspecified); workers only pull, publish their discoveries durably before `done()`, and the feeder re-claims once after in-flight hits zero before closing (the §11 #1 trap, one level down). `Recover` runs at every `Run` start on every surface: durable authority resets orphaned `claimed` rows (EC-01); the in-memory fallback (`memQueue`, bare library sinks) re-enqueues loader-supplied pending items. The runner's loader stopped materialising `Processed`/`Pending` (counts come from `PageCount`/`Count`) — the frontier-linear term is gone from the resume path too. Gates: `TestFrontierRAMSlopeFlat` (slope ≈ 0 over a 50k-URL frontier delta, with a detector arm proving the harness sees the linear slope when the queue capability is hidden), `TestMaxURLsTerminatesWithLargeAdmittedFrontier` (WP-21), `TestEveryURLFetchedExactlyOnce` (WP-08), `TestResumeRecoversOrphanedClaimedRows` (EC-01), `TestFrontierQueue_*` (store lifecycle; the v5 forward-migration and its `TestFrontierClaimedSeqForwardMigration` gate were retired 2026-07-05 once all installs reached v5 — DESIGN.md §5.3), `TestWorkPool_FeederNoBusyLoop` (WP-15), `TestWorkPool_HighFanoutSpillsNotBlocks` (WP-03, by construction).
...
- **Re-verified against `main` (commit f1b1d45):** every sink still present. The recently-merged
  `internal/queue` + `internal/runner` is a **whole-crawl job scheduler** (one crawl at a time then;
  up to `speed.max_concurrent_crawls` at once since #78, persisted across surfaces) — it does **not**
  touch per-URL memory. `internal/finalize` consolidated the post-crawl
...
**Per-crawl RAM bound (REALISED — issue #77):** the §5.2/§5.3 design targets
`O(threads + ready_buffer + bloom + small_counters)`, **independent of frontier and
crawled-page count** — and with the #77 feeder + bounded ready-buffer shipped, that
is the bound as built: the work queue is the durable `frontier` table, the in-RAM
window is `clamp(4·threads, 32, 1024)` items, records stream-and-drop, and the
visited set is the store. Gated by `TestFrontierRAMSlopeFlat` (slope ≈ 0 across a
50k-URL frontier delta; the in-RAM fallback arm shows the linear slope the gate
forbids). The former exception — `Resume.Admitted` bucket-counter rehydration —
is now streamed too (`store.EachAdmitted` → `frontier.BucketCounts`), so nothing
frontier-sized is retained even on a per-bucket-capped resume; gated by
`TestResumeBucketCounterRAMFlat`. See "Honest residuals".
...
7. **PageRank parity** (Phase 5): CSR-over-`links` == the original string-map PageRank — now **bit-exact** (was ~1e-9), because accumulation runs in a canonical sorted order. (The int32-`url→id` CSR was not built; sorted accumulation achieves the same determinism — see "Honest residuals".)
...
| **3** ✅ | DONE (re-scoped, then completed by #77). N persistent workers + atomic in-flight counter replacing `wg.Wait()` (`workpool.go`); `c.sem`/`process()` removed. Done-gate `TestGoroutinesBoundedByThreads`; invariants in `workpool_test.go`. The initially-deferred SQLite feeder + `claimed`/`seq` ready-buffer LANDED via issue #77 (see the progress note above): the queue is now a bounded window over the durable frontier, fed by a single claimer in `(depth, seq)` order. | **High** | — |
...
| **6** ✅ | DONE (issue #77). The "done" gate runs in the suite on the axis that mattered: `TestFrontierRAMSlopeFlat` asserts peak-RAM slope ≈ 0 on the **frontier** axis (two facet fixtures, discovered ≫ crawled), with a built-in detector arm — the same harness over the in-RAM fallback queue — that must show the linear slope, so the gate can never go silently blind. Goroutine bound: `TestGoroutinesBoundedByThreads` (Phase 3). Per-crawl RAM now flat on both axes. | Low | — |
...
| **8** ✅ | DONE (ALL surfaces — #78 finished what shipped CLI-only). Executor holds many crawls (`cur map[crawlID]`) with crawl-id `PauseCrawl/StopCrawl` + no-arg fan-out + a shared limiter; Dispatcher runs W drain loops (`queue.WithConcurrency`, atomic `ClaimNext` so no double-claim, wake-the-next, `Shutdown` waits all loops), `Cancel(jobID)`→`StopCrawl` (guarded against the startup-window empty crawlID, issue-#73 P18). Config `speed.max_concurrent_crawls`; `projects crawl-all --parallel N` runs N members at once under one global fetch cap. **#78 wired desktop + MCP:** both read the knob from the default profile at dispatcher construction (`runner.ProcessWiring`, restart to apply) and share ONE limiter when parallel; per-crawl addressing re-landed as a driven API (`SnapshotCrawl`/`Snapshots`/`CurrentAll` — the P16 removal was of the then-undriven version) plus `Dispatcher.AwaitCrawl` (per-job start handshake) and crawl-id MCP tool args / desktop events. Tests: GL-05/GL-17 (parallel/no-double-claim/burst), GL-09/GL-12 (registry under W loops), GL-13 (shutdown-all incl. SQLiteStore level), GL-18 (cap bound), GL-22 (store parity), GL-03/GL-04 (crawl-id control), X-02/X-04, `TestRunnerParallelStartsTwoCrawls`/`TestRunnerParallelGlobalCapProcessWide` (MCP), `TestDesktopParallelCrawlsEventsDoNotCross` (desktop), and a `queue.feature` parallel-drain scenario. | Med | ✓ |
...
| WP-03 ✅ | DONE (#77) | C | Sole-producer deadlock: last worker blocks on a *blocking* buffer send instead of spill-or-leave-durable. STRUCTURALLY IMPOSSIBLE as built: workers never push — discoveries go durable via `Enqueue` and only the feeder sends to the buffer. | `TestWorkPool_HighFanoutSpillsNotBlocks` — N=1, 10k children through a 32-item buffer, completes; plus `TestMaxURLsTerminatesWithLargeAdmittedFrontier`. |
...
| WP-08 ✅ | DONE (#77) | C | Feeder/worker claim race double-processes a row (non-atomic SELECT…LIMIT then UPDATE). As built the claim is ONE atomic `UPDATE … WHERE url IN (SELECT …) RETURNING`, and the feeder is the sole claimer. | `TestEveryURLFetchedExactlyOnce` (server-side per-URL hit counts, N=8, `-race`) + `TestFrontierQueue_BornClaimedInvisibleUntilEnqueued` (no re-claim). |
...
| WP-15 ✅ | DONE (#77) | M | Feeder busy-loops re-SELECTing on a full buffer (WAL churn) OR stalls with unclaimed rows; feeder↔worker handshake deadlock. As built: a full buffer blocks the send (no re-SELECT); an empty claim parks on a 1-slot poke channel (publish / inflight-zero wakes, never lost). | `TestWorkPool_FeederNoBusyLoop` (claim count frozen while parked) + every drain watchdog (`TestWorkPool_TreeNoEarlyTermination` 50× `-race`). |
| WP-21 ✅ | DONE (#77, semantics per the SHIPPED pool) | H | After max_urls, thousands of admitted `claimed=0` rows make "no unclaimed rows remain" never true → no termination. As built the feeder keeps draining them through the cheap over-budget path — each row is pulled, reserve-fails, and is `FrontierDone`'d, exactly like the pre-feeder queue — so the capped crawl still seals `completed` with an empty frontier (NOT "pending-for-resume": that described the unbuilt design; the product contract, pinned long before #77, completes a capped crawl). | `TestMaxURLsTerminatesWithLargeAdmittedFrontier` — MaxURLs=2 against 3k admitted rows: terminates, spends exactly the budget, frontier empty. |
...
| ID | R/G | Sev | Failure mode | Test (type) — asserts |
|---|---|---|---|---|
| EC-01 ✅ | DONE (#77) | C | Hard crash leaves `claimed=1` orphans; feeder's `WHERE claimed=0` skips them forever → silently lost. As built the reset is ENGINE-owned (`frontier.Queue.Recover`, called at every `Run` start before the feeder's first claim) — structural on every surface, not a per-loader call. | `TestResumeRecoversOrphanedClaimedRows` (every pending row forged claimed=1, resume through the production runner → page set == straight) + `TestFrontierQueue_RecoverResetsOrphanedClaims` (store unit: reset exactly-once; done rows stay gone). |
| EC-02 ✅ | DONE (issue-#73 P2) | C | A frontier row whose page was already written (crash between `Page` and `FrontierDone`) → re-fetched, re-charges budget on resume. FIXED in the SHIPPED pool: `PendingFrontier` excludes any frontier url present in `pages` (there is no `claimed` column — the original feeder design wasn't built). | `TestResume_ClaimedRowWithPageAlreadyWritten_NotRefetched` (runner integration) — server hit-count for the URL stays 1 across the resume; `TestPendingFrontier_ExcludesAlreadyCrawled` (store unit). |
...
| EC-05 ✅ | DONE (issue-#73 P1) | H | Partial finalize not idempotent: `SetStatus(completed)` was set MID-sequence, before depth/analysis. FIXED: status is `interrupted` until depth + analysis are durable, then sealed `completed`; a tail failure leaves it interrupted. | `TestFinalize_PartialCrash_Idempotent` (integration) — analysis forced to fail mid-tail leaves status != completed; recovery re-run is byte-identical to a clean finalize. |
...
| EC-07 ✅ | DONE (#77, frontier arm; DiscoveredFrom was already pinned) | H | `seq` per-session / from rowid collides across the resume boundary → DiscoveredFrom + pull order drift. As built: frontier `seq` is an explicit column (never rowid) from an atomic counter seeded at open from `MAX(seq)` over the LIVE rows, so session-2 admissions sort after every surviving pending row. DiscoveredFrom stability rides the separate `edges.seq` (`MaxEdgeSeq` continuation, Phase 2 — pinned by resume-equivalence). | `TestFrontierQueue_SeqMonotonicAcrossSessions` (store unit) + `TestResumeEquivalence`/`_ThroughRunner` (guards, stay green). |
...
| FIN-CSRID (EC28) ✅ | DONE (issue-#73 P4, re-scoped) | H | PageRank accumulated edge contributions in Go map-iteration order → non-associative float sum → link_score drift run-to-run. FIXED by accumulating in a canonical SORTED node/edge order (the int32 `url→id` CSR was not built; sorting achieves the same bit-stability). | `TestLinkScore_DeterministicAcrossRuns` (60 runs byte-identical) + `TestPageRankCSRParity` tightened to exact. |
...
**unaddressed plan blocker** WAS registry.db lock contention (GL-09) — closed by #78: the registry now opens
with WAL + `busy_timeout` + `_txlock=immediate` (a deferred SELECT→UPDATE upgrade returns SQLITE_BUSY
*bypassing* the busy handler — exactly what W loops hit), and the fresh-DB rollback→WAL switch is serialized
in-process; the dispatcher retries claim errors on a timer instead of parking (#74 N8).
...
| GL-09 ✅ | DONE (#78) | C | 7 | `registry.db` contention: busy_timeout alone did NOT cover the deferred-tx SELECT→UPDATE upgrade (immediate SQLITE_BUSY, busy handler bypassed) nor the fresh-DB rollback→WAL switch racing concurrent opens. Fixed: WAL + `_txlock=immediate` in the registry DSN + serialized open. | `TestRegistryDBNoLockErrorUnderConcurrentJobOps` (race, was genuinely RED) — W=4 doing ClaimNext+SetCrawlID+Finish+Enqueue+List; ZERO errors, every job terminal exactly once. |
...
| GL-08 ✅ | DONE (#78, completing #69's crawler-level pin) | H | 7 | Limiter constructed per-crawl (Option omitted on a path) → `SUM` across M is M·G not G. Also surfaced & fixed: out-of-band crawl-start fetches (llms.txt / sitemap walk) bypassed the cap (`crawler.fetchCapped`). | `TestGlobalLimiterBoundsFetchesAcrossCrawls` + `TestOutOfBandFetchesRespectGlobalCap` (crawler, the latter was RED) + `TestRunnerParallelGlobalCapProcessWide` (MCP wiring) + `TestParallelCrawlAllComposition` (CLI wiring) — observed global max ≤ G on every path. |
...
| GL-18 ✅ | DONE (#78) | H | 7/8 | No max-concurrent-CRAWLS knob → "crawl all project" starts all members; per-crawl fixed overhead (Bloom/buffers/DBs/conn pools) OOMs on a different axis than the fetch cap. | `TestMaxConcurrentCrawlsBounded` — 20 jobs, cap 4; peak in-flight exactly 4, every job claimed once and done. Knob help text carries the M×footprint sizing guidance. |
...
| GL-11 ✅ | DONE (#78) | H | 8 | `Snapshot` not crawl-id-addressed → desktop/MCP show the wrong crawl's live counters. | `Executor.SnapshotCrawl(id)`/`Snapshots()` replace the any-one `Snapshot()`; addressing pinned by `TestRunnerParallelStartsTwoCrawls` (MCP: crawl_status(id) returns THAT crawl's seed/state) and `TestDesktopParallelCrawlsEventsDoNotCross` (per-crawl progress streams). |
| GL-12 ✅ | DONE (#78) | C | 8 | Shared `registry.db` coarse writes (CreateCrawl/SetStatus/markTerminal) interleave/clobber under M crawls (same root as GL-09). | `TestParallelCrawlsRegistryWritesConsistent` (integration) — M=4 real crawls of distinct sizes; exactly M rows, all completed, each row carries its own site's counts. |
| GL-13 ✅ | DONE (#78; dispatcher-level pin landed with #69) | C | 8 | Whole-dispatcher `Shutdown` pauses only ONE crawl (single `doneCh`/`exec.Pause()`) → M−1 abandoned. | `TestShutdownPausesAllInFlightCrawls` (integration, dispatcher+SQLiteStore, M=3 real crawls) — ALL interrupted/resumable (crawl rows AND job rows), Shutdown waits for every loop, goroutines wind down. |
| GL-17 ✅ | DONE (#78 — proven absent: the wake-the-next chain re-arms the cap-1 channel before each run) | H | 8 | Single cap-1 `wakeCh` shared by W loops → a burst enqueue wakes one loop, throughput collapses to serial. | `TestWLoopsDrainConcurrentlyAfterBurstEnqueue` — W=3 parked loops, burst 3 jobs; all 3 run concurrently. |
...
| GL-22 ✅ | DONE (#78) | M | 8 | Parity drift: a bug exists in only one store backend (MemStore↔SQLiteStore). | `TestParallelControlParityAcrossStores` (table) — claim-once / addressed-pause / shutdown-all run over `{MemStore, SQLiteStore}`, identical behaviour. |
...
| X-02 ✅ DONE (#78) | Resume crawl A while B,C run: A's rehydration (perDepth/perSub, Bloom rebuild, `claimed=1→0` reset) happens under live contention on the shared limiter + registry. | `TestResumeUnderParallelLoad` (integration): A paused mid-crawl, resumed on a W=3 dispatcher while B/C run under one shared limiter — A finishes the whole site exactly once; B/C complete with full counts | **critical** |
...
| X-04 ✅ DONE (#78) | Crawl A hits MaxURLs and wants to finalize, but the F=1 slot is held by B → A's worker termination must not deadlock waiting for the finalize slot. | `TestMaxURLsTerminationWhileFinalizeSlotHeld` (integration): the test holds the F=1 slot; A parks on it (not done, not deadlocked) and completes the moment it frees | high |
