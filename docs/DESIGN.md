...
`bluesnake compare <prev> <curr>`: attaches both DBs, applies URL mapping regexes to previous, computes per-filter membership deltas (Added/New/Removed/Missing per SF semantics) and element change detection (title/desc/h1/word-count/depth/link-metrics/content-similarity/structured-data-types), writes a comparison report (terminal summary + exportable CSV/JSON). **Page-state deltas** (status-code and indexability changes for URLs in both crawls) are always computed, independent of the `compare.change_detection` element list — a page going 200→404 or indexable→noindex is core to what a re-crawl diff means, not an optional element.

**Enriched report + comparison cache (`compare.Report`, report.go).** The engine result decorated into a self-contained report (issue deltas with catalogue names/severities and both occurrence totals, added/removed page briefs, status-mix and indexability distributions; lists capped, counts exact). `compare.CachedReport` stores it in the registry's `comparisons` table keyed by the ordered `(prev, curr)` pair — a comparison of two finished crawls is deterministic, so it computes once and every revisit on ANY surface is a single registry read. Three surfaces serve the same cached report: the desktop Compare tab (auto-compares the two most recent runs on open; also feeds the project pulse §5.9), the MCP tools `compare_crawls` (two ids auto-ordered chronologically, or `site` for its two most recent completed crawls; `max_list` trims response lists) and `project_diff` (a project member's two latest *comparable* crawls), and the CLI (`bluesnake compare` computes flag-driven and uncached — its `--set compare.url_mapping` semantics differ from frozen-config reports; `projects diff` prints the engine result). Rows are a cache, never authority: purged whenever either side's content can change (crawl deleted, resumed, re-analyzed), replaceable via force/Recompute, and versioned so a payload-shape change reads as a miss.
...
- **Dual-mode comparison.** Per-competitor *over time* reuses the pairwise `compare` engine verbatim (same domain ⇒ meaningful URL/issue deltas). *Cross-competitor* is a new read-only **metric scorecard** (`scorecard.go`): site size, indexable rate, status-code mix, issue counts by severity, link score, near-dups (+ optional avg word count / Flesch / schema.org coverage via SQLite JSON functions). Cross-domain URL comparison is **not** offered — disjoint URL sets make it degenerate. All metrics are single-pass SQL aggregates over each crawl DB; `LoadPages` is never used (it would reintroduce the per-crawl memory blow-up). The desktop Comparison page surfaces Mode A automatically as per-member **pulse cards** (`MemberPulse`, desktop layer): a since-last-crawl delta reduced from the §5.7 comparison cache (pages added/gone, status & indexability flips, issue movement, top moved issues) plus a trend over the comparable history (URLs / issue counts per crawl, capped at 12 points) — the pairwise diff computes once per crawl pair and is shared with the crawl workspace's Compare tab. The pulse lives in `project_app.go`, not the project package, so the overlay's zero-core-changes contract holds.
